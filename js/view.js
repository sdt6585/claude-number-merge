...
class GameView {
      ...
      for (let row = 0; row < config.rows; row++) {
          for (let col = 0; col < config.cols; col++) {
              if (this.model.state.grid[row][col] !== null && 
                  this.model.state.grid[row][col].value === this.model.state.highestValue) {
  ...
  showLevelUp(levelsToIncrease) {
      this.levelUpDisplay.textContent = levelsToIncrease > 1 ? 
      ...
      for (let row = 0; row < config.rows; row++) {
          for (let col = 0; col < config.cols; col++) {
              if (this.model.state.grid[row][col] !== null) {
                  ...
                  this.model.state.grid[row][col].element.style.backgroundColor = 
                      this.model.getBlockColor(blockValue);
              }
          }
      }
  }
